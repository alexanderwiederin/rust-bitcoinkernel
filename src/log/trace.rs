...
impl<'a> ScriptTraceFrameRef<'a> {
    ...
    /// branch. Control-flow opcodes (`OP_IF`, `OP_NOTIF`, `OP_ELSE`, `OP_ENDIF`)
    /// processed inside an inactive branch also report `false`. Only meaningful
    /// in `ScriptTraceFrameKind::Step`] frames.
    ...
    ///
    /// Returns `-1` if the current script evaluation does not report errors
    /// (for example, internal callers such as signet block solution checks that
    /// evaluate scripts without requesting an error code).
...
impl<'a> ScriptEvalStackRef<'a> {
    ...
    /// stack is at `len() - 1`.
    pub fn item(&self, index: usize) -> Option<ScriptEvalStackItemRef<'a>> {
        if index >= self.len() {
            return None;
        }
        Some(unsafe {
