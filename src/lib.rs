...
pub fn verify(
    ...
    if !ret {
        let err = match status {
            btck_ScriptVerifyStatus_btck_SCRIPT_VERIFY_ERROR_TX_INPUT_INDEX => {
...
impl ScriptPubkey {
    /// Returns the raw script bytes as a vector.
    ///
    /// This creates a copy of the underlying script data.
...
impl TxOut {
    ...
    pub(crate) fn from_raw(ptr: *mut btck_TransactionOutput) -> TxOut {
        TxOut { inner: ptr }
    }
    ...
    /// Returns the script pubkey that defines how this output can be spent.
    ///
    /// The returned script pubkey is independent of this output's lifetime.
...
/// A Bitcoin transaction containing inputs and outputs.
///
/// Transactions can be created from raw serialized data or built programmatically.
/// They represent the atomic units of value transfer in Bitcoin.
...
impl Transaction {
    ...
    pub fn output_count(&self) -> u64 {
        unsafe { btck_transaction_count_outputs(self.inner) }
    }
    ...
    pub fn output_at(&self, index: u64) -> Result<TxOut, KernelError> {
        if index >= self.output_count() {
            return Err(KernelError::OutOfBounds);
        }
        let output = unsafe { btck_transaction_get_output_at(self.inner, index) };
        Ok(TxOut::from_raw(output))
    }
}
...
impl UnownedBlock {
    ...
    ///
    /// This serializes the entire block in network format.
    pub fn to_bytes(&self) -> Vec<u8> {
...
impl Block {
    ...
    /// # Errors
    /// Returns [`KernelError::OutOfBounds`] if the index is invalid.
    pub fn transaction_at(&self, index: u64) -> Result<Transaction, KernelError> {
    ...
    /// Returns the raw block data as bytes.
    ///
    /// This serializes the entire block (header + transactions) in the format
...
impl BlockSpentOutputs {
    ...
    pub fn size(&self) -> u64 {
        unsafe { btck_block_spent_outputs_size(self.inner) }
    }

    /// Returns the spent outputs for a specific transaction in the block.
    ///
    /// # Arguments
    ...
    pub fn transaction_spent_outputs(
    ...
    ) -> Result<TransactionSpentOutputs, KernelError> {
        let prev_out = unsafe {
        ...
        let res = TransactionSpentOutputs { inner: prev_out };
        Ok(res)
    }
}
...
impl TransactionSpentOutputs {
    ...
    pub fn size(&self) -> u64 {
        unsafe { btck_transaction_spent_outputs_size(self.inner) }
    }

    /// Returns the coin at the specified input index.
    ///
    /// # Arguments
    ...
    /// # Errors
    /// Returns [`KernelError::OutOfBounds`] if the index is invalid.
    pub fn coin(&self, coin_index: u64) -> Result<Coin, KernelError> {
        let coin = unsafe {
...
/// A coin (UTXO) that was spent as input to a transaction.
///
...
impl Coin {
    ...
    /// Returns the transaction output data for this coin.
    ///
    /// This includes the amount and script pubkey of the original output
    /// that was spent.
    ///
    /// # Errors
    /// Returns [`KernelError::Internal`] if the coin data is corrupted.
    pub fn output(&self) -> Result<TxOut, KernelError> {
