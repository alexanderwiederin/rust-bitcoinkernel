...
fn c_serialize<F>(c_function: F) -> Result<Vec<u8>, ()>
where
    F: FnOnce(
...
impl ScriptPubkey {
    pub fn serialize(&self) -> Result<Vec<u8>, KernelError> {
        ...
        .map_err(|_| {
            KernelError::Internal("Failed to serialize script to Bitcoin wire format".to_string())
        })
    }
}
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, KernelError> {
        ...
        .map_err(|_| {
            KernelError::Internal(
                "Failed to serialize transaction to Bitcoin wire format".to_string(),
            )
        })
    }
}
...
impl UnownedBlock {
    ...
    pub fn serialize(&self) -> Result<Vec<u8>, KernelError> {
        ...
        .map_err(|_| {
            KernelError::Internal("Failed to serialize block to Bitcoin wire format".to_string())
        })
    }
}
...
impl Chain {
    ...
    pub fn next(&self, entry: &BlockTreeEntry) -> Option<BlockTreeEntry> {
        let entry = unsafe { btck_chain_get_next_block_tree_entry(self.inner, entry.inner) };
        if entry.is_null() {
            return None;
        }
...
impl<T: Log + 'static> Logger<T> {
    ...
    pub fn new(mut log: T) -> Result<Logger<T>, KernelError> {
        let options = btck_LoggingOptions {
            ...
            always_print_category_levels: 0,
        };
