...
///
/// This is the primary interface for interacting with the Bitcoin chain,
...
impl ChainstateManager {
    ...
    ///
    /// # Arguments
    /// * `context` - The [`Context`] that this chainstate manager will use. The context
    ///   must remain valid for the lifetime of the chainstate manager.
    ...
    /// containing specific validation results, including whether the block was connected
    /// to the active chain and any error details if validation failed.
    ///
    /// # Example
    /// ```no_run
    /// # use bitcoinkernel::*;
