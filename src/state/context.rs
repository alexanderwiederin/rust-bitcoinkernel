...
use crate::{
    ffi::{
        c_helpers, BTCK_CHAIN_TYPE_MAINNET, BTCK_CHAIN_TYPE_REGTEST, BTCK_CHAIN_TYPE_SIGNET,
...
impl Context {
    ...
    pub(crate) fn as_ptr(&self) -> *mut btck_Context {
        self.inner
    }
}
...
mod tests {
    ...
    fn test_chain_type_debug() {
        assert_eq!(format!("{:?}", ChainType::Mainnet), "Mainnet");
        assert_eq!(format!("{:?}", ChainType::Testnet), "Testnet");
        assert_eq!(format!("{:?}", ChainType::Testnet4), "Testnet4");
        assert_eq!(format!("{:?}", ChainType::Signet), "Signet");
        assert_eq!(format!("{:?}", ChainType::Regtest), "Regtest");
    }

    #[test]
    fn test_chain_type_clone() {
        let original = ChainType::Mainnet;
        let cloned = original.clone();
        assert_eq!(original, cloned);
    }

    #[test]
    fn test_chain_type_copy() {
        let original = ChainType::Mainnet;
        let copied = original;
    ...
    fn test_chain_params_creation() {
        ...
        assert!(!mainnet_params.inner.is_null());

        let testnet_params = ChainParams::new(ChainType::Testnet);
        assert!(!testnet_params.inner.is_null());

        let testnet4_params = ChainParams::new(ChainType::Testnet4);
        assert!(!testnet4_params.inner.is_null());

        let signet_params = ChainParams::new(ChainType::Signet);
        assert!(!signet_params.inner.is_null());

        let regtest_params = ChainParams::new(ChainType::Regtest);
        assert!(!regtest_params.inner.is_null());
    }
    ...
    #[test]
    fn test_context_builder_with_chain_type() {
        let builder = ContextBuilder::new().chain_type(ChainType::Testnet);
        assert!(!builder.inner.is_null());
    }
    ...
    #[test]
    fn test_chain_type_repr_values() {
        assert_eq!(ChainType::Mainnet as u8, BTCK_CHAIN_TYPE_MAINNET);
        assert_eq!(ChainType::Testnet as u8, BTCK_CHAIN_TYPE_TESTNET);
        assert_eq!(ChainType::Testnet4 as u8, BTCK_CHAIN_TYPE_TESTNET_4);
        assert_eq!(ChainType::Signet as u8, BTCK_CHAIN_TYPE_SIGNET);
        assert_eq!(ChainType::Regtest as u8, BTCK_CHAIN_TYPE_REGTEST);
    }

    #[test]
    fn test_context_creation_different_chains() {
        let chain_types = vec![
            ChainType::Mainnet,
            ChainType::Testnet,
            ChainType::Testnet4,
        ...
        for chain_type in chain_types {
            let result = ContextBuilder::new().chain_type(chain_type).build();
    ...
    #[test]
    fn test_chain_params_send_sync() {
        fn assert_send<T: Send>() {}
        fn assert_sync<T: Sync>() {}

        assert_send::<ChainParams>();
        assert_sync::<ChainParams>();
    }
    ...
    fn test_context_across_threads() {
        ...
        let handle = thread::spawn(move || context.interrupt());

        let result = handle.join().expect("Thread panicked");
        assert!(result.is_ok() || matches!(result, Err(KernelError::Internal(_))));
    }
}
