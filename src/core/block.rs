...
//! # Core Types
//!
//! - `Block` - A complete Bitcoin block with header and transactions
...
//!
//! Except for `Block`, each owned type has a corresponding reference type (`BlockHashRef`,
//! `BlockSpentOutputsRef`, etc.) for zero-copy borrowing from the underlying C++ data structures.
//!
//! # Common Patterns
//!
...
//! let raw_block_data = include_bytes!("block.dat");
//! let block = Block::new(raw_block_data)?;
//!
//! // Get block hash
//! let hash = block.hash();
...
//!
//! Block hashes can be created from byte arrays and converted between formats:
//!
//! ```no_run
...
//!
//! When validating blocks or handling reorgs, you may need to examine
//! the outputs that were spent by a block's transactions:
//!
//! ```no_run
//! # use bitcoinkernel::{ChainstateManager, BlockTreeEntry, KernelError};
...
///
/// # fn example() -> Result<(), bitcoinkernel::KernelError> {
/// let raw_block = include_bytes!("block.dat");
...
impl Block {
    ...
    ///
    /// # fn example() -> Result<(), bitcoinkernel::KernelError> {
    /// let block_data = include_bytes!("genesis.dat");
    ...
    /// the block's unique identifier in the blockchain.
    ///
    /// # Examples
    /// ```no_run
    /// # use bitcoinkernel::Block;
    /// # fn example() -> Result<(), bitcoinkernel::KernelError> {
    /// # let block_data = vec![0u8; 100];
    /// # let block = Block::new(&block_data)?;
    /// let hash = block.hash();
    /// println!("Block: {}", hash);
    /// # Ok(())
    ...
    /// It implements [`ExactSizeIterator`], so you can query the remaining count.
    ///
    /// # Examples
    /// ```no_run
    /// # use bitcoinkernel::Block;
...
///
/// The iterator implements [`ExactSizeIterator`], allowing you to query the
/// number of remaining transactions at any point.
///
/// # Lifetime
...
pub trait BlockSpentOutputsExt: AsPtr<btck_BlockSpentOutputs> {
    ...
    ///
    /// Note: This excludes the coinbase transaction, which has no inputs and
    /// therefore no spent outputs.
    ///
    /// # Examples
    /// ```no_run
...
/// that were consumed (spent) by a block's transactions. This data is essential for:
/// - Validating that inputs reference valid unspent outputs
/// - Rolling back blocks during chain reorganizations
/// - Reconstructing historical UTXO set states
///
/// # Structure
///
/// The spent outputs are organized by transaction (excluding the coinbase, which
/// has no inputs). Each transaction's spent outputs correspond one-to-one with
/// its inputs, in the same order.
///
/// # Reading from Disk
///
...
///
/// The iterator implements [`ExactSizeIterator`], allowing you to query the
/// number of remaining transaction spent outputs at any point.
///
/// # Lifetime
...
///
/// The iterator implements [`ExactSizeIterator`], allowing you to query the
/// number of remaining coins at any point.
///
/// # Lifetime
