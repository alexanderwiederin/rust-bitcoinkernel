...
use libbitcoinkernel_sys::{
    ...
    btck_witness_stack_destroy, btck_witness_stack_get_item_at,
};
...
pub trait TxInExt: AsPtr<btck_TransactionInput> {
    ...
    ///
    /// # Errors
    /// Returns [`KernelError::SerializationFailed`] if serialization fails.
    ///
    /// # Examples
    /// ```no_run
    /// # use bitcoinkernel::{prelude::*, Transaction, KernelError};
...
///
/// This trait provides shared functionality for [`WitnessStack`] and [`WitnessStackRef`],
...
pub trait WitnessStackExt: AsPtr<btck_WitnessStack> {
    ...
    ///
    /// # Errors
    /// Returns [`KernelError::OutOfBounds`] if the index is invalid, or
    /// [`KernelError::SerializationFailed`] if serialization fails.
    ///
    /// # Examples
    /// ```no_run
    /// # use bitcoinkernel::{prelude::*, Transaction, KernelError};
...
impl Drop for WitnessStack {
    fn drop(&mut self) {
        unsafe { btck_witness_stack_destroy(self.inner) };
    }
}

/// A borrowed reference to a witness stack.
///
/// Provides zero-copy access to witness stack metadata (item count). It implements
